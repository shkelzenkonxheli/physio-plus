...
import { Building2, Images, LogOut, Pencil } from "lucide-react";
...
export function PhysioPanel() {
  ...
  const { data: physio } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, first_name, last_name, slug, status, bio, photo_url, physiotherapist_specializations(specialization_id)",
        )
  ...
  const { data: privateInfo } = useQuery({
    ...
    enabled: Boolean(physioId),
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_physio_private", {
        _physio_id: physioId as string,
      } as never);
      if (error) throw error;
  ...
  const { data: appointments, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("appointments")
        .select(
          "id, start_at, status, price, service_name, client_first_name, client_last_name, client_phone",
        )
        ...
        {physio ? (
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-8">
            <TabsList className="flex h-auto w-full justify-start gap-1 overflow-x-auto p-1 [-webkit-overflow-scrolling:touch]">
              <TabsTrigger value="terminet" className="shrink-0 whitespace-nowrap">
                Terminet
              </TabsTrigger>
              <TabsTrigger value="kalendari" className="shrink-0 whitespace-nowrap">
                Kalendari
              </TabsTrigger>
              <TabsTrigger value="kategorite" className="shrink-0 whitespace-nowrap">
                Kategoritë
              </TabsTrigger>
              <TabsTrigger value="sherbimet" className="shrink-0 whitespace-nowrap">
                Shërbimet
              </TabsTrigger>
              <TabsTrigger value="orari" className="shrink-0 whitespace-nowrap">
                Orari & ditët e lira
              </TabsTrigger>
